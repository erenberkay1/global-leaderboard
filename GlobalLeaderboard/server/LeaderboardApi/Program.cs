...
// ---------------------------------------------------------------------------
// 2) VERİTABANI (PostgreSQL - KALICI veri)
//    Bulutta DATABASE_URL ortam değişkeni kullanılır (Render / Neon / Supabase).
//    Lokal için appsettings.json -> ConnectionStrings:Default kullanılır.
// ---------------------------------------------------------------------------
var rawConn = Environment.GetEnvironmentVariable("DATABASE_URL")
              ?? builder.Configuration.GetConnectionString("Default");
var connectionString = BuildNpgsqlConnectionString(rawConn);
builder.Services.AddDbContext<LeaderboardDbContext>(options => options.UseNpgsql(connectionString));
...
// ---------------------------------------------------------------------------
// YARDIMCI: Bağlantı dizesini Npgsql formatına çevirir.
//   - "postgres://user:pass@host:port/db" (Render/Neon URL) -> key-value formata
//   - zaten key-value ise olduğu gibi bırakır
//   - boşsa lokal varsayılanı döner
// ---------------------------------------------------------------------------
static string BuildNpgsqlConnectionString(string? raw)
{
    if (string.IsNullOrWhiteSpace(raw))
    {
        // Lokal varsayılan (kendi makinende Postgres kuruluysa kullanılır)
        return "Host=localhost;Port=5432;Database=leaderboard;Username=postgres;Password=postgres";
    }
    ...
    if (raw.StartsWith("postgres://") || raw.StartsWith("postgresql://"))
    {
        ...
        return $"Host={uri.Host};Port={dbPort};Database={db};Username={user};Password={pass};SSL Mode=Require;Trust Server Certificate=true";
    }

    return raw; // zaten Host=...;Port=... formatında
}
