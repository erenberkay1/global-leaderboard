...
Veritabanı olarak PostgreSQL kullanılır. Lokal çalıştırmak için makinende bir
PostgreSQL kurulu olmalı (veya bir bulut Postgres bağlantısını `DATABASE_URL`
ortam değişkenine verebilirsin). Tablolar ilk çalıştırmada otomatik oluşur.
...
> Veriler PostgreSQL'de kalıcıdır. Bulutta sunucu yeniden başlasa bile skorlar
> korunur. Bağlantı, `DATABASE_URL` ortam değişkeninden okunur (Render / Neon /
> Supabase gibi herhangi bir Postgres ile çalışır).
